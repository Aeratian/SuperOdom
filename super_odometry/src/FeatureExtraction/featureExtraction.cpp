...
namespace super_odometry {
    ...
    featureExtraction::featureExtraction(ros::NodeHandle& nh) : nh_(nh) {
    }

    void featureExtraction::initInterface() {      
        if(!readGlobalparam(nh_))
        {
            ROS_ERROR("[super_odometry::featureExtraction] Could not read calibration. Exiting...");
            ros::shutdown();
        }
        std::cout << "0000000000000000000000000000000000000000000" << std::endl;
        if (!readParameters())
        {
            ROS_ERROR("[super_odometry::featureExtraction] Could not read parameters. Exiting...");
            ros::shutdown();
        }
        ...
        ROS_WARN("config_.skipFrame: %d", config_.skipFrame);
        ROS_INFO("scan line number %d \n", config_.N_SCANS);      
        ROS_INFO("use imu roll and pitch %d \n", config_.use_imu_roll_pitch);

        if (config_.N_SCANS != 16 && config_.N_SCANS != 32 && config_.N_SCANS != 64 && config_.N_SCANS != 4 && config_.N_SCANS != 128)
        {
            ROS_ERROR("only support velodyne, livox, ouster with 16, 32, 64 or 128 scan line! and livox mid 360");
            ros::shutdown();
        }
        ROS_INFO("[TartanAir] subscribe to %s", LASER_TOPIC.c_str());
        if (config_.sensor == SensorType::VELODYNE || config_.sensor == SensorType::OUSTER) {
            ROS_INFO("[TartanAir] subscribe to %s", LASER_TOPIC.c_str());
            subLaserCloud = nh_.subscribe(LASER_TOPIC, 2, 
                    &featureExtraction::laserCloudHandler, this);
        } else if (config_.sensor == SensorType::LIVOX) {
#if 0
            subLivoxCloud = nh_.subscribe<livox_ros_driver::CustomMsg>(ProjectName + "/livox/lidar", 10,
                        &featureExtraction::livoxHandler, this);
#endif
        }

        subImu = nh_.subscribe(IMU_TOPIC, 10, 
            &featureExtraction::imu_Handler, this);

        subOdom = nh_.subscribe(ODOM_TOPIC, 10, 
            &featureExtraction::visual_odom_Handler, this);

        pubLaserCloud = nh_.advertise<sensor_msgs::PointCloud2>(
            ProjectName+"/velodyne_cloud_2", 2);

        pubLaserFeatureInfo = nh_.advertise<super_odometry_msgs::LaserFeature>(
            ProjectName+"/feature_info", 2);

        pubBobPoints = nh_.advertise<sensor_msgs::PointCloud2>(
            ProjectName+"/bob_points", 2);

        pubPlannerPoints = nh_.advertise<sensor_msgs::PointCloud2>(
            ProjectName+"/planner_points", 2);

        pubEdgePoints = nh_.advertise<sensor_msgs::PointCloud2>(
        ...
        if (PUB_EACH_LINE)
        {
            ...
            {
                auto tmp_publisher_ = nh_.advertise<sensor_msgs::PointCloud2>(
    ...
    {   
        ...
        nh_.param("feature_extraction_node/scan_line", config_.N_SCANS, 4);
        nh_.param("feature_extraction_node/mapping_skip_frame", config_.skipFrame, 1);
        nh_.param("feature_extraction_node/blindFront", config_.box_size.blindFront, 0.1);
        nh_.param("feature_extraction_node/blindBack", config_.box_size.blindBack, -1.0);
        nh_.param("feature_extraction_node/blindLeft", config_.box_size.blindLeft, 0.1);
        nh_.param("feature_extraction_node/blindRight", config_.box_size.blindRight, -0.1);
        nh_.param("feature_extraction_node/use_dynamic_mask", config_.use_dynamic_mask, false);
        nh_.param("feature_extraction_node/use_imu_roll_pitch", config_.use_imu_roll_pitch, false);
        nh_.param("feature_extraction_node/min_range", config_.min_range, 0.2f);
        nh_.param("feature_extraction_node/max_range", config_.max_range, 130.0f);
        nh_.param("feature_extraction_node/filter_point_size", config_.filter_point_size, 3);
        nh_.param("feature_extraction_node/provide_point_time", config_.provide_point_time, 1);
        nh_.param("feature_extraction_node/debug_view", config_.debug_view_enabled, false);
        nh_.param("feature_extraction_node/imu_acc_x_limit", config_.imu_acc_x_limit, 1.0);
        nh_.param("feature_extraction_node/imu_acc_y_limit", config_.imu_acc_y_limit, 1.0);
        nh_.param("feature_extraction_node/imu_acc_z_limit", config_.imu_acc_z_limit, 1.0);
        
        std::string sensor_type;
        nh_.param("feature_extraction_node/sensor", sensor_type, std::string("livox"));
        
        ROS_INFO("[TartanAir] sensor_type: %s", sensor_type.c_str());

        if (sensor_type == "livox") {
                                                     ...
                                                     MapRingBuffer<pcl::PointCloud<point_os::PointcloudXYZITR>::Ptr> &lidarBuf)
    {
        ...
        {
            ROS_WARN_STREAM("meas_end_time < lidar_end_time ||"
                            " message order is not perfect! please restart velodyne and imu driver!");
            ROS_WARN("meas_end_time %f <  %f lidar_end_time", meas_end_time, lidar_end_time);
            ROS_WARN("All the lidar data is more recent than all the imu data. Will throw away lidar frame");
        ...
        {
            ROS_WARN("throw laser scan, only should happen at the beginning");
            lidarBuf.clean(lidar_start_time);
            ROS_WARN("removed the lidarBuf size % d, measureBuf size % d ", lidarBuf.getSize(), measureBuf.getSize());
            ROS_WARN("meas_start_time: %f > lidar_start_time: %f ", meas_start_time, lidar_start_time);
            return false;
        }
        else
        {
    ...
    featureExtraction::publishCloud(ros::Publisher thisPub, typename pcl::PointCloud<Point>::Ptr thisCloud,
                                         ...
                                         Eigen::Quaterniond q_w_original_l)
    {
        ...
        std::cout<<"lidar_start_time: "<<lidar_start_time<<std::endl;
        FeatureHeader.stamp = ros::Time().fromSec(lidar_start_time);
    ...
    void featureExtraction::undistortionAndFeatureExtraction()      
    {
        ...
        LASER_CAMERA_SYNC_SUCCESS = synchronize_measurements<nav_msgs::Odometry::ConstPtr>(visualOdomBuf, lidarBuf);
        ...
        if ((LASER_IMU_SYNC_SCCUESS == true or LASER_CAMERA_SYNC_SUCCESS == true) and lidarBuf.getSize() > 0)
        {
            ...
            {
                ROS_INFO("\033[1;32m----> Both IMU ,VIO laserscan are synchronized!.\033[0m");
                removePointDistortion<nav_msgs::Odometry::ConstPtr>(lidar_start_time, lidar_end_time, visualOdomBuf, lidar_msg);
            }
            ...
            {
                // ROS_INFO(this->get_logger(), "\033[1;32m----> IMU and laserscan is synchronized!.\033[0m");
        ...
        else if (imuBuf.empty())
        {
            ...
            ROS_INFO("\033[1;32m----> no IMU data, running LiDAR Odometry only.\033[0m");
        ...
        {
            ROS_WARN("sync unsuccessfull, skipping scan frame");
        }
    ...
    ImuMeasurement featureExtraction::parseImuMessage(const sensor_msgs::Imu::ConstPtr& msg) {
    ...
    void featureExtraction::updateImuOrientation(Imu::Ptr& imudata) {
        if (!imuBuf.empty()) {
            ...
            tf::Quaternion orientation_curr(imudata->q_w_i.x(),
                                        imudata->q_w_i.y(),
                                        imudata->q_w_i.z(),
                                        imudata->q_w_i.w());
            double roll, pitch, yaw;
            tf::Matrix3x3(orientation_curr).getRPY(roll, pitch, yaw);
            
            tf::Quaternion yaw_quat;
            yaw_quat.setRPY(0, 0, -yaw);
            tf::Quaternion first_orientation = yaw_quat * orientation_curr;
    ...
    void featureExtraction::imuInitialization(double timestamp) {
        ...
        if(lidarBuf.getFirstTime(lidar_first_time) && 
            ...
            if (timestamp - first_time > 1.0 && !IMU_INIT) {
                ...
                ROS_INFO("IMU Initialization Process Finish!");
            }
        }
    }

    void featureExtraction::imu_Handler(const sensor_msgs::Imu::ConstPtr& msg_in) {
        m_buf.lock();
    ...
    void featureExtraction::visual_odom_Handler(const nav_msgs::Odometry::ConstPtr &visualOdometry)
    {
        m_buf.lock();
        visualOdomBuf.addMeas(visualOdometry, visualOdometry->header.stamp.toSec());
        m_buf.unlock();
    }
    ...
    void featureExtraction::laserCloudHandler(const sensor_msgs::PointCloud2::ConstPtr& laserCloudMsg)
    {  
        ROS_INFO("[TartanAir] laserCloudHandler");
        ...
        if (config_.provide_point_time)
        {
            ...
            else if (config_.sensor == SensorType::OUSTER)
            {
                ...
                for (size_t i = 0; i < tmpOusterCloudIn->size(); i++)
                {
                    ...
                    utils::transformOusterPoints(&src, &dst, T_ouster_sensor);
                    dst.time = src.t * 1e-9f;
                }
            }
            else
            {
                ROS_ERROR("Unknown sensor type: %d", int(sensor));
                ros::shutdown();
            }
        }
        else
        ...
        manageLidarBuffer(pointCloud, laserCloudMsg->header.stamp.toSec());

        if(IMU_INIT==true or imuBuf.empty())
        {   
            ROS_ERROR("[TartanAir]jump inside to this function?");
...
#if 0
    void featureExtraction::livoxHandler(const livox_ros_driver2::CustomMsg::ConstPtr& msg)
    {   
        frameCount = frameCount + 1;
        if (frameCount % config_.skipFrame != 0)
            return; 
        ...
        if(config_.provide_point_time) {     
            ...
            ROS_ERROR("Please check yaml or livox driver to provide the timestamp for each point");
            ros::shutdown();
        }

        manageLidarBuffer(pointCloud, msg->header.stamp.toSec());
        ...
        double timestamp)
    {
        ...
        while (curLidarBufferSize >= 50) {
            ...
            lidarBuf.getFirstTime(lidar_first_time);
            lidarBuf.clean(lidar_first_time);
            ROS_WARN("Lidar buffer too large, dropping frame");
