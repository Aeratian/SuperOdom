...
namespace super_odometry {
    ...
    public:
        ...
        void mappingOptimization(Eigen::Vector3i &postion_in_locamap, Transformd start_tf, tf::Quaternion roll_pitch_quat,
                            const int laserCloudCornerStackNum, const int laserCloudSurfStackNum);

        void publishTopic(Eigen::Vector3i postion_in_locamap);

        void laserFeatureInfoHandler(const super_odometry_msgs::LaserFeature::ConstPtr& msgIn);

        void laserCloudRawDataHandler(const sensor_msgs::PointCloud2::ConstPtr& laserCloudRawdata);
        ...
        void getOdometryFromTimestamp(MapRingBuffer<nav_msgs::Odometry::Ptr> &buf, const double &timestamp,
                                 Eigen::Vector3d &T, Eigen::Quaterniond &Q);

        void extractRelativeTransform(MapRingBuffer<nav_msgs::Odometry::Ptr> &buf, Transformd &T_pre_cur, bool imu_prediction);
    ...
    private:
        ...
        ros::Subscriber subLaserRawdata;
        ros::Subscriber subLaserFeatureInfo;

        // Publishers
        ros::Publisher pubLaserCloudSurround;
        ros::Publisher pubLaserCloudMap;
        ros::Publisher pubLaserCloudPrior;
        ros::Publisher pubLaserCloudFullRes;
        ros::Publisher pubLaserCloudFullRes_rot;
        ros::Publisher pubLaserCloudFullResOusterWithFeatures;
        ros::Publisher pubLaserCloudFullResOuster;
        ros::Publisher pubLaserCloudRawRes;
        ros::Publisher pubOdomAftMapped;
        ros::Publisher pubOdomAftMapped_rot;
        ros::Publisher pubOdomAftMappedHighFrec;
        ros::Publisher pubLaserAfterMappedPath;
        ros::Publisher pubOptimizationStats;
        ros::Publisher pubLaserOdometryIncremental;
        ros::Publisher pubPreviousCloud;
        ros::Publisher pubPreviousPose;
        ros::Publisher pubprediction_source;
        ros::Publisher pubVIOPrediction; 
        ros::Publisher pubLIOPrediction;

        ros::Timer process_timer_;

        MapRingBuffer<nav_msgs::Odometry::Ptr> imu_odom_buf;
        MapRingBuffer<nav_msgs::Odometry::Ptr> visual_odom_buf;
        ...
        std::queue<sensor_msgs::PointCloud2> cornerLastBuf;
        std::queue<sensor_msgs::PointCloud2> surfLastBuf;
        std::queue<sensor_msgs::PointCloud2> realsenseBuf;
        std::queue<sensor_msgs::PointCloud2> fullResBuf;
        std::queue<sensor_msgs::PointCloud2> rawWithFeaturesBuf;
        std::queue<sensor_msgs::PointCloud2::Ptr> rawDataBuf;
        ...
        sensor_msgs::PointCloud2 priorCloudMsg;
        ...
        std::mutex mBuf;
        ros::Time timeLatestImuOdometry;
        ros::Time timeLastMappingResult;
